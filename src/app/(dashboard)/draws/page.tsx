              ...
              <div className="max-w-4xl mx-auto">
                  ...
                  <div className="bg-white/10 backdrop-blur-md p-6 rounded-2xl shadow-lg border border-white/20 mb-10">
                    <form onSubmit={handleAddDrawSubmit} className="flex flex-col gap-6">
                        ...
                        <div className="flex flex-col"><label className="mb-2 font-semibold text-white/80">Horarios</label><div className="flex items-center gap-2"><select value={newHour} onChange={e => setNewHour(e.target.value)} className="bg-white/20 text-white px-3 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-400 w-auto">{Array.from({length: 12}, (_, i) => i + 1).map(h => <option key={h} value={h.toString().padStart(2, '0')}>{h.toString().padStart(2, '0')}</option>)}</select><span className="font-bold">:</span><select value={newMinute} onChange={e => setNewMinute(e.target.value)} className="bg-white/20 text-white px-3 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-400 w-auto">{Array.from({length: 12}, (_, i) => i * 5).map(m => <option key={m} value={m.toString().padStart(2, '0')}>{m.toString().padStart(2, '0')}</option>)}</select><select value={newPeriod} onChange={e => setNewPeriod(e.target.value)} className="bg-white/20 text-white px-3 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-400 w-auto"><option value="AM">AM</option><option value="PM">PM</option></select><button type="button" onClick={handleAddHorario} className="flex items-center justify-center p-3 bg-gray-500/50 hover:bg-gray-500/70 transition-colors rounded-lg font-semibold disabled:opacity-50 ml-auto"><Plus className="w-6 h-6"/></button></div><div className="flex flex-wrap gap-2 mt-3">{horarios.map(h => (<div key={h} className="flex items-center gap-2 bg-green-500/30 text-green-200 px-3 py-1 rounded-full text-sm"><span>{h}</span><button type="button" onClick={() => handleRemoveHorario(h)}><XCircle className="w-5 h-5 text-red-400 hover:text-red-300"/></button></div>))}</div></div>
